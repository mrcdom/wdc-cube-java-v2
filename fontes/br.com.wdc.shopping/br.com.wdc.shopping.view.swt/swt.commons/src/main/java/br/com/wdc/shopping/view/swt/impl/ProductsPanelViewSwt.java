 ...
 */
public class ProductsPanelViewSwt extends AbstractViewSwt {
    ...
    public LongConsumer onOpenProduct;
    ...
    public ProductsPanelViewSwt(SwtApp app) {
        super("products-panel", app);
    ...
    public void doUpdate() {
        ...
        var products = state != null ? state.products : null;
    ...
    private void createProductCard(ProductInfo product) {
        ...
        card.addPaintListener(e -> paintProductCard(e.gc, card, product));

        card.addListener(SWT.MouseDown, _e -> {
            var pid = (Long) card.getData("productId");
            safeAction("openProduct", () -> { if (this.onOpenProduct != null) this.onOpenProduct.accept(pid); });
        });
    ...
    private void paintProductCard(GC gc, Canvas card, ProductInfo product) {
