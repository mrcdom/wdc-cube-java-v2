package br.com.wdc.shopping.view.swing;

import java.time.Duration;
import java.util.concurrent.ScheduledExecutorService;
import java.util.concurrent.TimeUnit;

import javax.swing.SwingUtilities;

import br.com.wdc.framework.commons.concurrent.ScheduledExecutor;
import br.com.wdc.framework.commons.function.Registration;
import br.com.wdc.framework.commons.function.ThrowingRunnable;

/**
 * Adapter that schedules tasks on a ScheduledExecutorService but ensures
 * execution happens on the Swing Event Dispatch Thread.
 */
public class ScheduledExecutorSwingAdapter implements ScheduledExecutor {

    private final ScheduledExecutorService service;

    public ScheduledExecutorSwingAdapter(ScheduledExecutorService service) {
        this.service = service;
    }

    @Override
    public Registration execute(ThrowingRunnable command) {
        var future = this.service.schedule(() -> SwingUtilities.invokeLater(command), 0, TimeUnit.MILLISECONDS);
        return () -> future.cancel(true);
    }

    @Override
    public Registration schedule(ThrowingRunnable command, Duration delay) {
        var future = this.service.schedule(() -> SwingUtilities.invokeLater(command), delay.toMillis(),
                TimeUnit.MILLISECONDS);
        return () -> future.cancel(true);
    }

    @Override
    public Registration scheduleAtFixedRate(ThrowingRunnable command, Duration initialDelay, Duration period) {
        var future = this.service.scheduleAtFixedRate(() -> SwingUtilities.invokeLater(command),
                initialDelay.toMillis(), period.toMillis(), TimeUnit.MILLISECONDS);
        return () -> future.cancel(true);
    }

    @Override
    public Registration scheduleWithFixedDelay(ThrowingRunnable command, Duration initialDelay, Duration delay) {
        var future = this.service.scheduleWithFixedDelay(() -> SwingUtilities.invokeLater(command),
                initialDelay.toMillis(), delay.toMillis(), TimeUnit.MILLISECONDS);
        return () -> future.cancel(true);
    }
}
