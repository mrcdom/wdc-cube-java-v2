package br.com.wdc.shopping.view.remote.javaclient;
...
import br.com.wdc.shopping.view.remote.javaclient.model.SecretContext;
...
public final class ClientCrypto {
    ...
    public static SecretContext generateSecret(String appSKey) {
        try {
            ...
            // Server-side uses PBEKeySpec(password.toCharArray()) where password = new String(pwdBytes, UTF-8) = pwd
            // So PBKDF2 password = pwd.toCharArray() = pwdBytes (all ASCII, equivalent)
            ...
            // Flutter: base64Encode(pwdBytes) → UTF-8 bytes → BigInt (unsigned) → encrypt → base36
